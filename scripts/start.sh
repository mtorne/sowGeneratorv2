...
"$VENV_PYTHON" -m uvicorn main:app --host 0.0.0.0 --port 8000 > "$BACKEND_LOG" 2>&1 &
BACKEND_PID=$!
echo "$BACKEND_PID" > "$BACKEND_PID_FILE"

echo "Starting app backend..."
cd "$BASE_DIR" || exit 1
"$VENV_PYTHON" -m uvicorn app.main:app --host 0.0.0.0 --port 8001 > "$APP_BACKEND_LOG" 2>&1 &
APP_BACKEND_PID=$!
echo "$APP_BACKEND_PID" > "$APP_BACKEND_PID_FILE"

echo "Starting frontend..."
cd "$BASE_DIR/frontend" || exit 1
"$VENV_PYTHON" -m http.server 8080 --bind 0.0.0.0 > "$FRONTEND_LOG" 2>&1 &
FRONTEND_PID=$!
